...
class TripletAllLoss(nn.Module):
    ...
    def forward(self, source, s_labels, target=None, t_labels=None, margin=0):
        ...
        # Jaccard_Sim = (s_labels.mm(t_labels.t())) / (
        #             s_labels.sum(1).reshape((batch_size, 1)) + t_labels.sum(1).reshape((1, batch_size)) - s_labels.mm(
        #         t_labels.t()))
        # Jaccard_Dis = 1 - Jaccard_Sim.expand((batch_size, batch_size, batch_size))

        mask = _get_triplet_mask(s_labels, t_labels)

        # triplet_loss = mask * triplet_loss * Jaccard_Dis
